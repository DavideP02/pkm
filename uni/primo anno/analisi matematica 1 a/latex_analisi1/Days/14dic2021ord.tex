...
\subsubsection{Applicazioni del teorema di De L'H\^opital}

Segue la dimostrazione del Teorema Di Darboux (di cui viene data una differente versione):

\paragraph{Teorema di Darboux (\Roman{thmdarbouxdi})} Sia $ f $ derivabile in $ I\setminus\{x_0\} $, $ f $ continua in $ x_0 $
\begin{align*}
...
\proprieta{}{\label{proprietaettte}
    Si ha che \begin{equation}
        T_{n}[g'(x)] = T_{n+1}' [g(x)]  
    \end{equation} dove \[
        T_{n}[f(x)] = \sum_{k=0}^{n} \frac{f^{(k)}(x_0)}{k!}(x-x_0)^{k}
    \]
}
...
\begin{itemize}
    ...
    Abbiamo effettuato una sostituzione: $ t=h/x $, mentre l'ultimo è un limite notevole % ATOD aggiungere nella formula
    ...
    \end{multline*}
    \consecfoottext{1}{eleojhdsiuhduu}{Si sostituisce $ t=h/x $}
    \footnotetext{Limite notevole: $ \lim_{h\to 0} \frac{\ln (1+t)}{t}=1 $}
...
\dimostrazione{diderfunccomi}{
    ...
    \end{multline*} allora \begin{equation*}
        \lim_{h\to 0} \frac{g \bigl(f(x+h)\bigr)-g\bigl(f(x)\bigr)}{h} = g'(f(x))\, f'(x)\qedd
    \end{equation*}
    \todo{Manca l'iterazione del processo}
}
...
\esercizio{
   ...
   Da svolgere % ESERCIZIO risolvere 
}{}
...
\todo{Manca il grafico di $ f $ e di $ f^{-1} $}
}
...
\esempi{}{
    ...
    \[
        x\neq 0\qquad f'(x)=\sin \frac{1}{x}+x\,\cos x\,\frac{1}{x^{2}}=\parentesi{\footnotemarkk{adelfi}}{\sin \frac{1}{x}}+\parentesi{\footnotemark}{\cos x\,\frac{1}{x}}
    \]
    \consecfoottext{1}{adelfi}{oscilla tra $ \pm 1 $ per $ x\to 0 $}
    \footnotetext{oscilla tra $ \pm \infty $ per $ x\to 0 $}

    \todo{Manca il limite del rapporto incrementale}
    ...
    \todo{Manca la fine dell'esercizio}
\end{enumerate}
}

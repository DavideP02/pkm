\definizione{
	Data\marginnote{4 nov 2021} $ F:V\to W $ applicazione lineare tra spazi vettoriali su uno stesso campo, il rango di F ($ \rank F $) è la dimensione di $ F(V) $
}
...
\dimostrazioneprop{retro}{
	...
	$\implies$ $ F^{-1}(K) $ sottospazio vettoriale di $ W $\qed
}
...
\dimostrazione{kern}{
	\begin{itemize}
		...
		$\implies$ $ v_1=v_2 $, quindi $ F $ è iniettiva.\qed
	\end{itemize}
}
...
\esercizio{
	...
	$ g(V) $ ha dimensione 2. Per esercizio si trovi una base di $ g(V) $ % ESERCIZIO svolgere esercizio
}

\notazione{
	Spesso l'immagine di una funzione lineare $ F $ si indica con $ \im(F) $
}
...
\dimostrazione{chedde}{
	\begin{enumerate}
		\item $ \dim V =\dim W $, $ \dim V = \dim \ker (F)+\dim \im(F) $
		
		$ \implies$ $ \dim W = \dim \ker (F)+\dim \im(F) $

		\begin{itemize}
		\item Se $ F $ è suriettiva 
		
		$\implies$ $ \dim W = \dim \im(F) $

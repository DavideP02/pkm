%TODO aggiungere da qualche parte cosa significa bilineare (lineare in ogni variabile)

% In ogni spazio vettoriale (di dimensione >=1) esistono infiniti prodotti scalari 
% TODO trovare un modo dove sistemarli
...
\end{align*}
\proprieta{di $ \cdot $}{
    \begin{enumerate} % TODO riguardare dagli appunti 
...
\definizione{}{
    Sia $ V $ uno spazio vettoriale complesso. Un \textit{prodotto Hermitiano} su $ V $ è una funzione $ \cdot: V\times V \to \C$ tale che % TODO riguardare dagli appunti 
    \begin{enumerate}
...
\osservazione{
    ...
    $\implies$ $ v \cdot (w_1+w_2)=v \cdot w_1 + v \cdot w_2$ % TODO manca un pezzo
}
...
\esercizio{Si dimostri che $ \cdot  $ è un prodotto Hermitiano su $ \C^{n} $}{Da fare %TODO risolvere esercizio
}{}
...
\dimostrazione{herm1}{
    Data per esercizio %TODO svolgere la dimostrazione
}
...
\esempio{
    La base canonica di $ \C^{n} $ %TODO qual è la base canonica di $ \C^n $?
